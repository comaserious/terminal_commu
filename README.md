...
런처에서 `↑` / `↓`로 항목을 이동하고 `Enter`로 선택합니다. 사이트를 고른 뒤 추천 게시판을 열거나 URL을 직접 입력할 수 있습니다. URL 입력 단계의 `Esc`는 이전 단계로 돌아갑니다.
...
이미지와 동영상은 다운로드하거나 터미널에 표시하지 않고 `[이미지]`, `[동영상]`으로 나타냅니다. 디시콘은 `[디시콘]`, 아카라이브 이모티콘은 `[이모티콘]`으로 나타냅니다.
