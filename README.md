...
캐시는 `~/.cache/commu/cache.db`에 저장됩니다. 직접 입력한 URL 기록은 `~/.cache/commu/url-history.json`에 저장됩니다. 캐시 키에는 사이트와 게시판 식별자가 포함되어 서로 다른 커뮤니티의 같은 글 번호가 충돌하지 않습니다. 이전 `~/.cache/fmk-reader/` 캐시는 재사용하거나 자동 삭제하지 않습니다.
...
- **FMKorea**: 모바일 버전(`m.fmkorea.com`)을 사용하여 Cloudflare 차단을 우회합니다.
- **아카라이브**: `curl-cffi` 라이브러리를 사용하여 TLS 핑거프린팅을 우회하고 Cloudflare 차단을 회피합니다. 이 라이브러리가 설치되지 않은 경우 기본 HTTP 클라이언트를 사용하며 Cloudflare 차단 시 오류가 표시됩니다.
- **디시인사이드**: 이미 모바일 버전(`m.dcinside.com`)을 사용 중입니다.

JavaScript/WASM challenge나 Turnstile 같은 고급 보안 검증은 우회하지 않습니다. 브라우저 쿠키나 challenge 토큰도 사용하지 않습니다. 사용 가능한 캐시가 없으면 오류가 표시됩니다.
