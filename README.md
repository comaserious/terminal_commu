...
시작 메뉴에서 `↑` / `↓`로 이동하고 `Enter`로 선택합니다. 업무 데이터 소스를 고른 뒤 자동 업무 피드를 동기화하거나 외부 업무 리소스 URL을 직접 입력할 수 있습니다. 이전에 직접 입력했던 URL이 있으면 같은 데이터 소스의 최근 업무 리소스로 함께 표시되며, 선택하면 바로 연결됩니다. URL 입력 단계의 `Esc`는 접속 방식 선택으로, 접속 방식 선택 단계의 `Esc`는 데이터 소스 선택으로 돌아갑니다.
